    ...
    for (const user of demoUsers) {
      ...
      const userResult = await client.query(
        `INSERT INTO users (email, password_hash, first_name, last_name, avatar) 
         VALUES ($1, $2, $3, $4, $5) 
         RETURNING id`,
        [user.email, passwordHash, user.firstName, user.lastName, user.avatar]
      );
...
export const seedJobs = async () => {
  ...
  try {
    ...
    // Insert jobs with generated UUIDs
    for (const job of jobs) {
      const jobId = randomUUID(); // Generate a new UUID for each job
      await client.query(
        `INSERT INTO jobs (
...
export const seedAll = async () => {
  try {
    ...
    // Seed users first (this clears existing data)
    await seedUsers();
